...
parms0 <- list(
  ...
  epc.proj_sla = c(11.5, 34), # bijoor et al 2014
  epc.max_root_depth = c(0.21, 1.52),
  epc.waring_pa = c(0.2, 1),
  epc.gl_smax = c(0.004, 0.012), # reyes et al. 2017
  epc.day_leafoff = c(265, 305),
  epc.day_leafon = c(79, 119),
  epc.ndays_expand =  c(10, 40),
  epc.ndays_litfall =  c(15, 40),
  epc.alloc_prop_day_growth = c(0.2, 0.5),
  epc.storage_transfer_prop = c(0.3, 0.7))
...
parm_strat %>% dplyr::filter(parm_strat$`"veg_parm_ID"`==3) %>% 
  group_by(`"month"`,`"year"`) %>% 
  summarise(lai = mean(`"height"`)) %>% 
  mutate(yrmo = as.yearmon(paste(`"year"`,`"month"`, sep="-"))) %>% 
  ggplot() + geom_point(aes(x=yrmo, y=lai))
...
parm_veg_state = parm_strat %>% 
  ...
  mutate(yrmo = as.yearmon(paste(`"year"`,`"month"`, sep="-"))) %>% 
  rename(lai = `"lai"`, month = `"month"`, run=`"run"`) %>% 
  group_by(run, month) %>% 
  summarise(lai = mean(lai)) %>% 
  group_by(run) %>% 
...
ggplot(parm_veg_state) + geom_boxplot(aes(x=as.factor(lai_mo), y=value, group=lai_mo)) + 
  facet_wrap('name', scales='free') + 
  coord_flip()

# filter by month 
parm.mo_filt_c3 = parm_veg_state %>% 
  dplyr::filter(lai_mo == 12 | lai_mo <= 3)

length(unique(parm.mo_filt$group_id))
