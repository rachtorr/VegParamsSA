---
title: "temperature differences - proj climate model outputs"
output: html_document
---

## changes made:

- for both def files, changed epc.topt to 12 (deg c)
- changed epc.tcoef = 0.1 (changed from 0.2)
...
- **hist:** actual data 
- **hot:** increase by 2 deg C 
- **hotter:** increase by 4.6 deg C, the largest positive difference in clim projections
- **lower:** decrease by 4.2 deg C, the largest negative difference in clim projections
...
ggplot(rmonths) + geom_line(aes(x=as.factor(month), y=plant_resp, col=climmod, group=climmod)) + facet_wrap('code')
...
```{r climscen, echo=F}
clim = read.csv("../../../GitHub/climfiles/sb_all_clim_UPDATED.csv")
split <- str_split(clim$cal, pattern = "-")
clim <- clim %>%
  mutate(year = as.numeric(lapply(split, "[[",1))) %>%
  mutate(month = as.numeric(lapply(split, "[[", 2))) %>%
  mutate(day = as.numeric(lapply(split, "[[", 3))) %>% mkwy()
...
t_anom <- clim %>% 
  ...
  group_by(wy) %>%
  summarize_at(vars(tave),funs(mean)) 
...
readout %>% dplyr::filter(code=='quag' & run==1) %>% 
  ...
  ggplot() + geom_line(aes(x=wy, y=precip)) + ggtitle("precip")

ggplot(readout[readout$wy<2018,]) + geom_boxplot(aes(x=as.factor(wy), y=lai, fill=climmod)) + facet_wrap('code', scales='free')

ggplot(readout[readout$wy<2018,]) + geom_boxplot(aes(x=as.factor(wy), y=gpsn, fill=climmod)) + facet_wrap('code', scales='free')

readout %>% group_by(wy, code, climmod) %>% 
  summarize(lai=mean(lai)) %>%
  ggplot() + 
  geom_col(data=readout[readout$run==1,], aes(x=wy, y=precip/500), fill='grey') + 
  geom_line(aes(
    x=wy, y=lai, col=climmod)) + 
