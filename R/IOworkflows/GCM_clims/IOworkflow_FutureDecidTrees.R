...
setwd("~/Documents/patches/")

# read in the parameters 
setwd("R/tree_outputs/")
dec <- read.csv("decid_params.csv") %>% dplyr::select(-X)
...
output_variables[2,] <- data.frame("cdg", "gwseasonday", stringsAsFactors = FALSE)
output_variables[3,] <- data.frame("cdg", "lfseasonday", stringsAsFactors = FALSE)
...
#########################################################################
# make tec file - need to add part from generate_input_files
#########################################################################
...
for(i in seq_along(spec)){
  ...
  option_sets_all = sample_n(dec[dec$code==spec[i],], 50)
  #option_sets_all = dec
...
# bb is ID, so also applies to what run #
   # for (aa in seq_along(option_sets_def_par)){
    ...
    for (bb in seq_along(option_sets_def_par[[1]]$group_id)){
        ...
        cmd = sprintf("mv ../worldfiles/EucPatch.world.0.Y2010M8D1H1.state ../worldfiles/EucWorld.spun70")
      system(cmd)
