...
p1 = readRDS("../filtered_sets.rds")
p2 = readRDS("../filtered_sets2.rds") %>% mutate(epc.waring_pa=0.8)
parm_df = rbind(p1, p2)[14:24]
...
world = "../worldfiles/test_msr_irr_2stratum.VegOnly.world" 
whdr = "../worldfiles/test_msr_tgC3.hdr"
tec= "../tecfiles/tec.spinup"
str = "1989 10 1 1"
end = "2000 10 1 1"
cmd = "-g -vmort_off -of watbal_of.yml -climrepeat"
  ...
  for (bb in seq_along(1:n)){
    ...
    for (cc in seq_along(option_sets_def_par)){
      ...
      rhessys_command(rhessys_version = rhv,
                      ...
                      world_hdr_file = whdr,
                      tec_file = tec,
                      flow_file = "../flowtables/test_msr.2stratum.50.flow",
                      start_date = str,
                      end_date = end,
      ...
      tmp = mkdate(output2) %>% 
        mutate(run = bb,
               type = "c3",
               veg = factor(veg_parm_ID, labels = c("grass","tree")))

      
      if(bb==1 & aa==1){
        write.table(as.data.frame(tmp), file=paste(outdir, "control_strat.csv", sep=""), append=F, row.names = FALSE)
      }else{
        write.table(as.data.frame(tmp), file=paste(outdir, "control_strat.csv", sep=""), append=T, col.names = FALSE, row.names = FALSE)}
...
parm_strat = read.csv(paste(outdir, "control_strat.csv", sep=""), sep="") 

# plot time series 

parm_strat %>% dplyr::filter(veg=="grass") %>% 
  group_by(yd, run, patchID) %>% 
  summarise(lai = mean(trans)) %>% 
  ggplot() + geom_point(aes(x=yd, y=lai, col=run)) + facet_wrap('patchID') + scale_color_viridis_c()
...
# compare params 
parm_df$run = seq(1:10)

bind = parm_strat %>% dplyr::filter(patchID==60853702) %>% 
  group_by(run) %>% 
  summarise(mt= min(trans), mn = min(cdf.psn_to_cpool)) %>% 
  inner_join(parm_df, by="run")




parm_strat %>% dplyr::filter(parm_strat$`"veg_parm_ID"`==3) %>% 
  group_by(`"year"`, `"run"`,`"patchID"`) %>% 
  summarise(npp=sum(`"npp"`)) %>% 
  group_by(`"year"`, `"patchID"`) %>% 
  summarise(npp=mean(npp)) %>% 
  ggplot() + geom_line(aes(`"year"`,y=npp, col=as.factor(`"patchID"`)))
