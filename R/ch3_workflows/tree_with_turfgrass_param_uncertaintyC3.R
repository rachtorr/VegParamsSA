...
pat <- read.csv("~/VegParamsSA/out/msr_on_2strat/pat.csv", sep="")
bas =  read.csv("~/VegParamsSA/out/msr_on_2strat/bas.csv", sep="")
bas %>% group_by(date, irr, veg_area) %>% summarise(wb=mean(watbal)) %>% ggplot() + geom_point(aes(x=date, y=wb)) + facet_grid(irr~veg_area)
...
parm_strat %>% dplyr::filter(parm_strat$`"veg_parm_ID"`==3) %>% 
  ...
  ggplot() + geom_point(aes(x=`"date"`, y=lai))
...
parm_strat %>% dplyr::filter(parm_strat$`"veg_parm_ID"`==3) %>% 
  ...
  ggplot() + geom_line(aes(x=`"yd"`, y=psn))


# get month when LAI is the highest 
...
#  dplyr::filter(lai_mo == 6)
parm.mo_filt_c3 = parm_veg_state %>% 
   dplyr::filter(month_psn >= 3 & month_psn <= 5)
length(unique(parm.mo_filt_c3$group_id))
...
parm_veg_out.filt = parm_veg_out[parm_veg_out$group_id %in%
                                   parm.mo_filt_c3$group_id,]

# lai and all parameters
ggplot(parm_veg_out.filt) + geom_point(aes(x=value, y=height)) + facet_wrap('name', scales="free") +
...
parm_veg_out.filt %>%
  ggplot() + geom_point(aes(x=value, y=trans)) + facet_wrap('name', scales="free") +
  geom_smooth(aes(x=value, y=trans))
...
ps %>% group_by(`"yd"`) %>% dplyr::filter(`"veg_parm_ID"`==3) %>% 
  summarise(trans = mean(`"trans"`),
            psn = mean(`"cdf.psn_to_cpool"`),
            lai = mean(`"epv.proj_lai"`)) %>% 
  ggplot() + geom_line(aes(x=`"yd"`, y=lai))

ps %>% dplyr::filter(`"veg_parm_ID"`==3) %>% 
  ggplot() + geom_boxplot(aes(x=`"month"`, y=`"epv.proj_lai"`, group=`"month"`))

saveRDS(parm_veg_out.filt_ht, paste(outdir, "c3_output_parm.rds", sep=""))
