---
title: "patch family surface transfer balances: oak tree"
date: "2023-09-09"
...
wb_bas = read.csv("../out/oak50/msr_on_bas.csv") %>% 
  mkdate() %>% 
  mutate(canopy_snow_stored = snow_stored,                                           canopy_rain_stored = rain_stored) %>%
  watbal_basin_of()

ggplot(wb_bas) + geom_point(aes(x=date, y=watbal)) + ggtitle("tree surf_g = 1; 
                                                             basin water balance")

wb_bas = read.csv("../out/oak50/msr_off_bas.csv") %>% 
  mkdate() %>% 
  mutate(canopy_snow_stored = snow_stored,                                           canopy_rain_stored = rain_stored) %>%
  watbal_basin_of()

ggplot(wb_bas) + geom_point(aes(x=date, y=watbal)) + ggtitle("tree surf_g = 0; 
                                                             basin water balance")
...
```{r patch, echo=FALSE}
wb_p = read.csv("../out/oak50/msr_on_p.csv") %>% mkdate() 
...
msr flag is on, but oak tree land use file has surf_g = 0 (cannot gain surface transfer) and shading_flag = 0

```{r tree, echo=FALSE}

oak = read.csv("../out/oak50/msr_off_p.csv") %>% mkdate() %>%
  mutate(msr="off",
         id = factor(patchID, 
...
ggplot(tocomp) + geom_boxplot(aes(x=msr, y=surface_transfer, col=id))

(ggplot(tocomp) + geom_boxplot(aes(x=id, y=streamflow, col=msr)))/(ggplot(tocomp) + geom_line(aes(x=date, y=streamflow, col=msr, linetype=id)))

(ggplot(tocomp) + geom_boxplot(aes(x=id, y=evap, col=msr)))/(ggplot(tocomp) + geom_line(aes(x=date, y=evap, col=msr, linetype=id)))

(ggplot(tocomp) + geom_boxplot(aes(x=id, y=rootzone.S, col=msr)))/(ggplot(tocomp) + geom_line(aes(x=date, y=rootzone.S, col=msr, linetype=id)))

(ggplot(tocomp) + geom_boxplot(aes(x=id, y=rootzone.field_capacity, col=msr)))/(ggplot(tocomp) + geom_line(aes(x=date, y=rootzone.field_capacity, col=msr, linetype=id)))
...
nomsr = read.csv("../out/oak50/msr_off_strat.csv") %>% mkdate() %>%
  mutate(msr="off",
         id = factor(patchID, 
                     levels = c(60853701, 60853702), 
                     labels=c("tree", "paved")))

strat = read.csv("../out/oak50/msr_on_strat.csv") %>% mkdate() %>% 
  mutate(msr="on",
                id = factor(patchID, 
                     levels = c(60853701, 60853702), 
                     labels=c("tree", "paved"))) %>% 
...
#### after 10 years run, daily values 

```{r boxplots, echo=FALSE}

tree = strat %>% dplyr::filter(patchID == 60853701 & year==2004)
...
(ggplot(tree) + geom_boxplot(aes(x=msr, y=rootc, col=msr))+ggtitle("root C")) + (ggplot(tree) + geom_boxplot(aes(x=msr, y=stemc, col=msr))+ggtitle("stemC")) + (ggplot(tree) + geom_boxplot(aes(x=msr, y=cs.leafc, col=msr))+ggtitle("leafC"))
...
is gain is turned off for both patches, where does any transfer go? water is not balanced 
