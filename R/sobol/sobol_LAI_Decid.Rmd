...
sobolout <- readRDS("../../out/decid/allsim/jansenoutput_decid.rds")
...
read <- readin_selected_vars(dir = "../../out/decid/allsim", vars)

#read_lmf <- read %>%
  # mutate(lmf=(leafc+dead_leafc+leafc_store)/1000/plantc) %>% 
  # dplyr::filter(year> 2008 & year<2012) %>% 
  # group_by(run) %>% 
  # summarize_at(vars(lmf), list(mean))

#ggplot(read_lmf) + geom_boxplot(aes(x=lmf))
...
params <- read.csv("../../out/decid/allsim/veg.decid_all_options.csv")
...
I like to start with these density plots - really showing the distribution of outcomes for the different parameter sets (n=1064). I started with this small value because it runs faster to compile these plots and get an idea of if there are any differences. This plot tells me there are a solid set of runs that included reasonable LAI for all three years, but not that there was recovery by 2017.  
...
# ggplot(sann) + geom_jitter(aes(x=`2011`, y=`2017`, col=change1)) + geom_abline(intercept=0, slope=1) + 
#   coord_fixed()
...
running_plantc <- test2 %>% dplyr::filter(year > 2000) %>%
  ...
  ggtitle("runnin average plant C")
running_lai
running_plantc
...
# ggplot(sann) + geom_density(aes(x=epc.branch_turnover)) +  geom_density(data=filr2, aes(x=epc.branch_turnover, col='filtered'))
# 
# ggplot(sann) + geom_density(aes(x=epc.min_percent_leafg)) +
#   geom_density(data=filr2, aes(x=epc.min_percent_leafg, col='filtered'))
# 
# ggplot(sann) + geom_density(aes(x=epc.waring_pa)) + 
#   geom_density(data=filr2, aes(x=epc.waring_pa, col='filtered'))
# 
# ggplot(sann) + geom_density(aes(x=epc.proj_sla)) + 
#   geom_density(data=filr2, aes(x=epc.proj_sla, col='filtered'))
...
#sens_lai <- sensitivity::tell(sobolout, read_ann$change)
...
sens_res <- sensitivity::tell(sobolout, read_yr$resil)
...
res_S <- sens_res$S %>% 
  mutate(params = rownames(sens_res$S)) %>%
  ggplot() +
  geom_col(aes(y=original, x=params, fill=original),position="dodge") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
res_T <- sens_res$T %>% 
  mutate(params = rownames(sens_res$T)) %>%
  ggplot() +
  geom_col(aes(y=original, x=params, fill=original),position="dodge") + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
alloc <- ggplot(sann) + 
  geom_jitter(aes(x=`2011`, y=`2017`, col=epc.alloc_stemc_leafc))
alloc 
...
ggplot(sann) + geom_jitter(aes(x=`2011`, y=`2017`, col=epc.leaf_turnover))

turnover <- ggplot(sann) + 
  geom_jitter(aes(x=`2011`, y=`2017`, col=epc.leaf_cn))
turnover 

sla <- ggplot(sann) + 
  geom_jitter(aes(x=`2011`, y=`2017`, col=epc.proj_sla))
sla 

ggplot(sann) + 
  geom_jitter(aes(x=`2011`, y=`2017`, col=epc.gl_smax))

ggplot(sann) + 
  geom_jitter(aes(x=`2011`, y=`2017`, col=epc.root_growth_direction))

# ggplot(sann) + 
#   geom_jitter(aes(x=`2011`, y=`2017`, col=nitrif_parm_smax))
```
