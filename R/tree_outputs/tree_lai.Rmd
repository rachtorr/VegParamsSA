...
# load in mikes data 
rs_dat <- read.csv("~/Google Drive/My Drive/plant_params/alonzo_data_df.csv")
...
trees_rs <- rs_dat %>% dplyr::filter(
    ...
    class==34 ) #EUGL
## remove outliers 
trees_rs_rm <- trees_rs[!trees_rs %in% boxplot.stats(trees_rs$carbobperarea)$out,]
...
ggplot() +
  ...
  geom_boxplot(data=trees_rs_rm, aes(x="data", y=carbobperarea, fill='alonzo')) + facet_wrap('code') + 
