...
compare_lai <- rbind(
  dfyr2 %>% ungroup() %>% 
        ...
        mutate(data="rhessys", lai = as.numeric(lai)),
  trees_rs_rm %>%
        dplyr::select(lai, code) %>% 
        mutate(data="data"))
...
```{r params}
p_dec <- readRDS("plra_params.rds")
p_eug <- readRDS("eugl_params.rds")
p_ev <- readRDS("evg_broadleaf_params.rds")
p_con <- readRDS("pica_conifer_params.rds")

p_dec2 <- cbind(p_dec[,14:53], p_dec$code)
p_eug2 <- cbind(p_eug[,14:53], p_eug$code)
...
colnames(p_eug2) <- p_names
colnames(p_ev2) <- p_names
colnames(p_con2) <- p_names

all_p <- rbind(p_dec2, p_eug2, p_ev2, p_con2)
