...
readin_selected_vars <- function(vars, dir){
    ...
    tomelt <- as.data.frame(p) %>% mutate(date=as.Date(paste(year,month,day, sep="-"))) %>% 
      dplyr::select(c(-day, -month, - year, -basinID)) 
    
    toprint <- melt(as.data.frame(tomelt), id.var='date')
  ...
  colnames(df) <- c("day","run","date",vars)
  return(df)
}
...
vars = c("height",
         ...
         "plantc",
         "rootdepth")
read <- readin_selected_vars(vars,  dir="~/../../scratch/ratorres/out/allsim/", start_date="2002-09-30", end_date = "2012-09-30" )
