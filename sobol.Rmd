...
sobolout <- readRDS("../../out/RHESSysIOinR_output/allsim/sobolEffoutput.rds")
...
#   test = read.csv(paste(dir,vars[1], sep = "/"))
#   days = nrow(test)
#   n = ncol(test)
#   df = data.frame(day = rep(1:days, n), run=rep(1:n, each=days))
#   for(i in 1:length(vars)){
#     p = read.csv(paste(dir,vars[i], sep = "/"))
#     melt <- melt(p)
#     colnames(melt) <- c("var",paste(vars[i]))
# 
#     df <- cbind(df, melt[,2])
#   }
#   colnames(df) <- c("day","run",vars)
#   return(df)
# }
...
vars = c("height",
         ...
         "plantc",
         "rootdepth")
read <- readin_selected_vars(dir = "../../out/RHESSysIOinR_output/allsim", vars, start_date = "2002-09-30", end_date="2012-09-30")
